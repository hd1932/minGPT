...
The NameDataset takes the pretraining_dataset defined in run.py
...
0. The idx argument of __getitem__ retrieves the element of self.data
...
2. Break the (truncated) document into three substrings:
  ...
  In other words, it chooses three strings prefix, masked_content and suffix
    such that prefix + masked_content + suffix = [the original document].
  The length of [masked_content] is random, and 1/4 the length of the
    truncated document on average.

3. Rearranges these substrings into the following form:
...
4. Use masked_string to construct the input and output example pair. To
...
5. Making use of the vocabulary that is defined, encodes the resulting input
and output strings as Long tensors and returns the resulting data point.

----------------
Examples of input-output pairs (x, y):
...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        doc_len = random.randint(4, int(self.block_size * 7 / 8))
        document = document[:doc_len]
        mean_len = round(len(document) / 4)
        mask_len = random.randint(1, max(1, min(doc_len - 1, 2 * mean_len)))
        start = random.randint(0, doc_len - mask_len)
        prefix = document[:start]
        content_masked = document[start:start + mask_len]
        suffix = document[start + mask_len:]
        string_masked = f"{prefix}{self.MASK_CHAR}{suffix}{self.MASK_CHAR}{content_masked}"
        padding_needed = self.block_size - len(string_masked)
        string_masked += self.PAD_CHAR * padding_needed
        x = torch.tensor([self.stoi.get(c, 0) for c in string_masked[:-1]], dtype=torch.long)
        y = torch.tensor([self.stoi.get(c, 0) for c in string_masked[1:]], dtype=torch.long)
        return x, y
        
        # raise NotImplementedError

"""
Code under here is for debugging purposes.
"""
