...
class DownProjectBlock(nn.Module):
    ...
    Initializes similarly to the regular tranformer Block class,
    while using the CausalCrossAttention layer instead of the regular
    CausalSelfAttention layer.
    
    Initializes the parameter for the basis vectors `self.C`.
    Initializes `self.C` with appropriate dimensions and xavier_uniform initalization.
    
    self.C is 1 x bottleneck_dim x n_embd. Need the first dimension 
    ...
    def forward(self, x_input):
        """Performs cross-attention between x_input and self.C.
        Uses the layernorm layers on C, and then on the input to the MLP.
        Adds residual connections
        """        
        C_ln = self.ln1(self.C)
        x = self.C + self.attn(x_input, C_ln)
        x = x + self.mlp(self.ln2(x))
        return x
...
class UpProjectBlock(nn.Module):
    ...
    Initializes similar to the regular transformer Block class,
    while using the CausalCrossAttention layer instead of the regular
    CausalSelfAttention layer.
    """
    def __init__(self, config):
    ...
    def forward(self, y, x_input):
        """Performs cross-attention between previous layer's output y and
        x_input. 
        Uses the layernorm layers on y, and then on the input to the MLP.
        """
        y_ln = self.ln1(y)
        x = x_input + self.attn(y_ln, x_input)
        x = x + self.mlp(self.ln2(x))
        return x
